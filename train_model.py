...
def test(model, test_loader, criterion, device, epoch_no, hook):
    ...
    Parameters:
        ...
        hook (smdebug.pytorch.Hook): The debugger hook to use.
...
def train(model, train_loader, criterion, optimizer, device, epoch_no, hook):
    ...
    Args:
        ...
        epoch_no (int): The current epoch number.
        hook (smdebug.pytorch.Hook): The debugger hook object for debugging.
...
def main(args):
    ...
    # Instantiate a hook and register it with the model.
    ...
    # Instantiate a loss criterion and register it with the hook.
    ...
    for epoch_no in range(1, args.epochs + 1):
        print(f"Epoch {epoch_no} - Starting Training phase.")
        model = train(model, train_data_loader, loss_criterion, optimizer, device, epoch_no, hook)
        print(f"Epoch {epoch_no} - Starting Testing phase.")
...
if __name__=='__main__':
    ...
    #Add all the command-line arguments needed to train the model.
